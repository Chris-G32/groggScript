...
tokenIterator parseTypeName(const tokenIterator &it)
{
    ...
    {
        DEBUG_LOG(std::string("parsed a type name: ") + GroggScript::tokenTypeToString(it->token))
        return std::next(it);
        break;
    }
    default:
...
tokenIterator parseVarDeclaration(const tokenIterator &it)
{
    ...
    tokens.push_back(*localIt);
    std::advance(localIt, 1);
    if (localIt->token == TokenType::STRING_VALUE || localIt->token == TokenType::TRUE || localIt->token == TokenType::FALSE || localIt->token != TokenType::INTEGER)
    {
        tokens.push_back(*localIt);
        std::advance(localIt, 1);
    }
    ...
    {
        DEBUG_LOG("Encountered unsupported token, floats, expr and symbols to do" + GroggScript::tokenTypeToString(localIt->token))
        throw std::runtime_error("ERR");
    }
    DEBUG_LOG("PARSED VAR DECLARATION")
    for (const auto a : tokens)
    {
        DEBUG_LOG(std::string("<") + GroggScript::tokenTypeToString(a.token) + std::string(">"));
        DEBUG_LOG(a.value);
    }
    return localIt;
}
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        generateAST(tokenizer.getTokens());
    }
